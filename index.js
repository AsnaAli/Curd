...
app.get('/getFilter',async(req,res)=>{
  let response=await pool.query(`SELECT * FROM todolist.todolist where done=$1`,[req.body.done])
...
app.get('/getTask',async(req,res)=>{
  let response=await pool.query(`SELECT * FROM todolist.todolist where id=$1`,[req.body.id])
  console.log(response)
...
app.put('/updateDone', async(req, res) => {
  let updateuser = await pool.query(`UPDATE todolist.todolist
                   SET done = $2
